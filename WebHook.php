...
class WebHook extends Base
{
	...
	public function __construct(){
		...
		$this->loggerInfo->pushHandler(new StreamHandler('webhook_logs/info3.log', Logger::INFO));

		$this->loggerErrors = new Logger('errors');
		$this->loggerErrors->pushHandler(new StreamHandler('webhook_logs/errors3.log', Logger::DEBUG));
	}
	...
	public function processing(){
		$request = $_POST;
		$this->loggerInfo->info('processing', [
			'$_POST' => $_POST,
			'request' => $request,
		]);
		if (isset($request["tel"])) {
			...
			$this->loggerInfo->info('processingOfCreationLead', [
				'request' => $request,
			]);
			/**
	...
	public function processingOfUpdateLead(){
		...
		if (!empty($normalizedPhone)) {
			$json = $this->findLeadByPhone($normalizedPhone);
		}
	...
	public function processingOfCreationLead(){
		...
		if (!isset($request["Phone"]) || !isset($request["Name"])) {
			...
			if (isset($request["IDWEB"])) {
				if (!empty($normalizedPhone)) {
					$json = $this->findContactByPhone($normalizedPhone);
				}
				if (!empty($email) && (empty($normalizedPhone) || $json->total == 0)) {
